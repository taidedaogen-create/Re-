...
export class VoxelEngine {
  ...
  private recreateMesh() {
    ...
    this.buildInstancedMesh();
    // Immediate draw to prevent flickering
    this.draw();
  }
  ...
  public rebuild(targetModel: VoxelData[]) {
    ...
    // 1. DYNAMIC EXPANSION: If target needs more voxels, add them.
    if (targetModel.length > this.voxels.length) {
        const currentCount = this.voxels.length;
        const diff = targetModel.length - currentCount;
        const newVoxels: SimulationVoxel[] = [];
        
        for(let i=0; i<diff; i++) {
            // Spawn new voxels from below floor to hide the "creation"
            // They will fly up to their spots.
            newVoxels.push({
                id: currentCount + i,
                x: (Math.random() - 0.5) * 20,
                y: CONFIG.FLOOR_Y - 5, 
                z: (Math.random() - 0.5) * 20,
                color: new THREE.Color(0xcccccc), // Temporary color
                vx: 0, vy: 0, vz: 0, rx: 0, ry: 0, rz: 0,
                rvx: 0, rvy: 0, rvz: 0
            });
        }
        ...
        // We must recreate the mesh to accommodate new count
        this.recreateMesh();
        this.onCountChange(this.voxels.length);
    }

    const available = this.voxels.map((v, i) => ({ index: i, color: v.color, taken: false }));
    // Reset mappings with nulls
    ...
    targetModel.forEach(target => {
        ...
        if (bestIdx !== -1) {
            ...
            mappings[available[bestIdx].index] = {
                ...
                color: new THREE.Color(target.color), // Store target color
                delay: h * 600
            };
        }
    });
    ...
    for (let i = 0; i < this.voxels.length; i++) {
        if (!mappings[i]) {
            mappings[i] = {
                x: this.voxels[i].x, 
                y: CONFIG.FLOOR_Y + 0.5, // Drop to floor
                z: this.voxels[i].z,
                color: this.voxels[i].color, // Keep original color
                isRubble: true, 
                delay: 0
            };
        }
  ...
  private updatePhysics() {
    if (this.state === AppState.DISMANTLING) {
        ...
        this.voxels.forEach((v, i) => {
            ...
            // Handle Rubble behavior
            if (t.isRubble) {
                 // Simple physics for rubble to settle
                 if (v.y > CONFIG.FLOOR_Y + 0.5) {
                     v.y -= 0.5;
                 } else {
                     v.y = CONFIG.FLOOR_Y + 0.5;
  ...
  private animate() {
    ...
    // Always draw during rebuilding to show color changes
